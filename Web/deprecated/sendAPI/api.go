...
func main() {
    app := fiber.New()
    var errClient error
    rpcName := utils.GetENV("RPC_USER")
    rpcPass := utils.GetENV("RPC_PASS")
    rpcHost := utils.GetENV("RPC_HOST")
    rpcPort, err := strconv.Atoi(utils.GetENV("RPC_PORT"))
    if err != nil {
        utils.ReportMessage(err.Error())
        panic(err)
        return
    }
    client, errClient = coind.New(rpcHost, rpcPort, rpcName, rpcPass, false, 60)
    if errClient != nil {
