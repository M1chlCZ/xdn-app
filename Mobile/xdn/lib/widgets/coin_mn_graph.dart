...
class CoinMNGraph extends StatefulWidget {
  final MasternodeGraphResponse? stake;
  final int type;
  final Function(bool touch) blockTouch;

  const CoinMNGraph({Key? key, this.stake, required this.type, required this.blockTouch}) : super(key: key);

  @override
  CoinMNGraphState createState() => CoinMNGraphState();
}

class CoinMNGraphState extends State<CoinMNGraph> {
  double _leftTitlesInterval = 1;
  var _touch = false;
  MasternodeGraphResponse? _stakes;
    ...
    try {
      ...
      if (_stakes != null) {
        ...
        List<StakeData>? dataPrep = List.generate(_stakes!.rewards.length, (i) {
          return StakeData(
            date: _dateParse(_stakes!.rewards[i].day.toString(), _stakes!.rewards[i].hour, _dropdownValue),
  ...
  AxisTitles _bottomTitles() {
    return AxisTitles(
      ...
      interval: getInterval(_dropdownValue),
    ));
  }

  double getInterval(int value)  {
    if (value == 0) {
      return 300;
    } else if (value == 2) {
      return 4;
