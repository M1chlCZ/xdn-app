...
func StartRemoteMasternode(nodeID int, mnKey string) {
	...
	//XDN
	_, errMNstart := coind.WrapDaemon(*daemon, 125, "masternode", "start")
	if errMNstart != nil {
...
func ScanMasternodes() {
	...
	active, errScript := exec.Command("bash", "-c", fmt.Sprintf("systemctl --user is-active %s", "XDN")).Output()
	if strings.TrimSpace(string(active)) != "active" {
		_, _ = exec.Command("bash", "-c", fmt.Sprintf("systemctl --user restart %s", "XDN")).Output()
